...
void Sigprocmask(int how, const sigset_t *set, sigset_t *oldset)
{
	...
	unix_error((char*)"Sigprocmask error");
    return;
}
...
void Sigemptyset(sigset_t *set)
{
	...
	unix_error((char*)"Sigemptyset error");
    return;
}
...
{ 
    if (sigfillset(set) < 0)
	unix_error((char*)"Sigfillset error");
    return;
}
...
void Sigaddset(sigset_t *set, int signum)
{
	...
	unix_error((char*)"Sigaddset error");
    return;
}
...
void Sigdelset(sigset_t *set, int signum)
{
	...
	unix_error((char*)"Sigdelset error");
    return;
}
...
int Sigismember(const sigset_t *set, int signum)
{
	...
	unix_error((char*)"Sigismember error");
    return rc;
}
